...
class PrepareStripeVoucherRefundResponse(PrepareVoucherRefundResponse):
    purchase_id: str = Field(pattern=HEX32_PATTERN)
    operation_revision: int = Field(ge=1)


class CompleteStripeVoucherRefundRequest(CompleteVoucherRefundRequest):
...
class StripeVoucherTerminalChainEvidence(ApiModel):
    action: int = Field(ge=1, le=4)
    spend_bundle_id: str = Field(pattern=HEX32_PATTERN)
    terminal_evidence_hash: str = Field(pattern=HEX32_PATTERN)
    terminal_voucher_coin_id: str = Field(pattern=HEX32_PATTERN)
    series_input_coin_id: str = Field(pattern=HEX32_PATTERN)
    series_input_parent_coin_id: str = Field(pattern=HEX32_PATTERN)
    series_output_coin_id: str = Field(pattern=HEX32_PATTERN)
    series_output_inner_puzzle_hash: str = Field(pattern=HEX32_PATTERN)
    deed_output_coin_id: str | None = Field(
        default=None,
...
class PresaleStore:
    ...
    def _create_schema(self) -> None:
        self._conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS stripe_voucher_records_v3 (
              terms_hash TEXT NOT NULL REFERENCES presale_series_v2(terms_hash),
              serial INTEGER NOT NULL,
              deed_launcher_id TEXT NOT NULL,
              purchase_id TEXT NOT NULL UNIQUE,
              ...
              vault_launcher_id TEXT NOT NULL,
              vault_p2_puzzle_hash TEXT NOT NULL,
              stripe_receipt_hash TEXT NOT NULL UNIQUE,
              payment_evidence_id TEXT NOT NULL UNIQUE,
              state TEXT NOT NULL CHECK (state IN (
                'PENDING_ISSUANCE','ISSUANCE_SUBMITTED','ESCROWED',
                'REFUNDING','REFUNDED','REDEEMING','REDEEMED'
              )),
              funding_bundle_id TEXT UNIQUE,
              purchase_launcher_coin_id TEXT UNIQUE,
              purchase_launcher_puzzle_hash TEXT,
              issuance_bundle_id TEXT UNIQUE,
              voucher_launcher_id TEXT UNIQUE,
              voucher_output_coin_id TEXT UNIQUE,
              receipt_coin_id TEXT UNIQUE,
              series_input_coin_id TEXT UNIQUE,
              series_output_coin_id TEXT UNIQUE,
              issuance_confirmed_height INTEGER,
              terminal_action INTEGER,
              terminal_evidence_hash TEXT,
              terminal_bundle_id TEXT UNIQUE,
              terminal_voucher_coin_id TEXT UNIQUE,
              terminal_series_input_coin_id TEXT UNIQUE,
              terminal_series_output_coin_id TEXT UNIQUE,
              terminal_deed_output_coin_id TEXT UNIQUE,
              terminal_confirmed_height INTEGER,
              created_at INTEGER NOT NULL,
              updated_at INTEGER NOT NULL,
              PRIMARY KEY (terms_hash, serial),
              UNIQUE (terms_hash, deed_launcher_id)
            );
            CREATE INDEX IF NOT EXISTS idx_stripe_voucher_v3_state
              ON stripe_voucher_records_v3(terms_hash, state, serial);
    ...
    def get(self, identifier: str) -> dict[str, Any]:
        ...
        stripe_vouchers = [
            self._render_stripe_voucher(item, result)
            for item in self._conn.execute(
                """
                SELECT * FROM stripe_voucher_records_v3
                WHERE terms_hash=? ORDER BY serial
                """,
                (result["termsHash"].lower(),),
            ).fetchall()
        ]
        result["vouchers"] = sorted(
            [*legacy_vouchers, *stripe_vouchers],
            key=lambda item: int(item["serial"]),
        )
        return result
    ...
    def vouchers_for_vault(self, vault_launcher_id: str) -> list[dict[str, Any]]:
        ...
        stripe_rows = self._conn.execute(
            """
            SELECT * FROM stripe_voucher_records_v3
            WHERE vault_launcher_id=?
            ORDER BY created_at DESC, serial
        ...
        for row in stripe_rows:
            series = self._get_series(row["terms_hash"])
            voucher = self._render_stripe_voucher(row, series)
            deed = next(
                (
                    item
                    for item in series["terms"]["deeds"]
    ...
    def prepare_stripe_voucher(
        ...
        *,
        ...
        """Persist the one canonical Voucher V3 derived from Stripe success."""

        series = self.get(terms_hash)
        if series["state"] != "PRESALE":
            raise ValueError("Stripe voucher series is not in PRESALE")
        terms = _series_program(series["terms"])
        deed = next(
            (
                item
                for item in series["terms"]["deeds"]
                if str(item["deedLauncherId"]).lower()
                == _hex32(artifact.deed_launcher_id)
            ),
            None,
        )
        if deed is None:
            raise ValueError("Stripe purchase does not target this voucher series")
        if (
            artifact.collection_id != terms.collection_id
            or artifact.base_amount_minor != int(deed["basePriceMinor"])
            or artifact.technology_fee_bps != terms.technology_fee_bps
            or artifact.technology_fee_minor
            != int(deed["technologyFeeMinor"])
            or artifact.subtotal_minor != int(deed["grossPriceMinor"])
            or artifact.protocol_treasury_puzzle_hash
            != terms.trusted_protocol_treasury
        ):
            raise ValueError("Stripe purchase differs from governed presale terms")
        serial = int(deed["ordinal"])
        if original_payer != stripe_original_payer(artifact):
            raise ValueError("Stripe payer commitment is not canonical")
        ...
        observed_at = int(receipt.evidence.observed_at)
        validate_stripe_voucher_purchase(
            series=terms,
            voucher=voucher,
            artifact=artifact,
        ...
        commitment_json = voucher_commitment_v3_to_json(voucher)
        purchase_id = _hex32(artifact.purchase_id)
        commitment_hash = _hex32(voucher.commitment_hash)
        with self.txn() as cur:
            existing = cur.execute(
                """
                SELECT * FROM stripe_voucher_records_v3
                WHERE purchase_id=? OR commitment_hash=?
                """,
                (purchase_id, commitment_hash),
            ).fetchone()
            if existing is not None:
                if (
                    existing["purchase_id"] != purchase_id
                    or existing["commitment_hash"] != commitment_hash
                    or existing["commitment_json"] != _json(commitment_json)
                ):
                    raise ValueError(
                        "Stripe voucher purchase is bound to different evidence"
                    )
                return self._render_stripe_voucher(existing, series)
            occupied_v2 = cur.execute(
                """
                SELECT 1 FROM voucher_records_v2
                WHERE terms_hash=? AND (serial=? OR deed_launcher_id=?)
                """,
                (
                    series["termsHash"].lower(),
                    serial,
            ...
            occupied_v3 = cur.execute(
                """
                SELECT 1 FROM stripe_voucher_records_v3
                WHERE terms_hash=? AND (serial=? OR deed_launcher_id=?)
                """,
                (
                    series["termsHash"].lower(),
            ...
            if occupied_v2 is not None or occupied_v3 is not None:
                raise ValueError("the governed presale deed is already assigned")
            cur.execute(
                """
                ...
                (
                    ...
                    _hex32(artifact.deed_launcher_id),
                    purchase_id,
                    _hex32(voucher.global_payment_id),
                    commitment_hash,
                    _json(commitment_json),
                    _hex32(original_payer),
                    _hex32(artifact.vault_launcher_id),
                    _hex32(artifact.vault_p2_puzzle_hash),
        ...
        return self._render_stripe_voucher(row, series)

    def stripe_voucher_by_purchase(self, purchase_id: str) -> dict[str, Any]:
        row = self._conn.execute(
            "SELECT * FROM stripe_voucher_records_v3 WHERE purchase_id=?",
            (purchase_id.lower(),),
        ).fetchone()
        if row is None:
            raise KeyError(purchase_id)
        return self._render_stripe_voucher(
            row,
            self._get_series(str(row["terms_hash"])),
        )

    def stripe_voucher(self, terms_hash: str, serial: int) -> dict[str, Any]:
        series = self._get_series(terms_hash)
        row = self._conn.execute(
            """
            SELECT * FROM stripe_voucher_records_v3
    ...
    def pending_stripe_issuance(
        self,
    ) -> list[tuple[dict[str, Any], dict[str, Any]]]:
        rows = self._conn.execute(
            """
            SELECT v.terms_hash, v.purchase_id
            FROM stripe_voucher_records_v3 v
        ...
        return [
            (
                self._get_series(str(row["terms_hash"])),
                self.stripe_voucher_by_purchase(str(row["purchase_id"])),
            )
            for row in rows
        ]

    def record_stripe_issuance_funding(
        self,
        purchase_id: str,
        *,
        ...
        with self.txn() as cur:
            ...
            if row is None:
                raise KeyError(purchase_id)
            exact_retry = (
                row["funding_bundle_id"] == funding_bundle_id.lower()
                and row["purchase_launcher_coin_id"]
                == purchase_launcher_coin_id.lower()
                and row["purchase_launcher_puzzle_hash"]
                == purchase_launcher_puzzle_hash.lower()
            )
            if not exact_retry:
                if row["state"] != "PENDING_ISSUANCE" or any(
                    row[field]
                    for field in (
                        "funding_bundle_id",
                    ...
                    raise ValueError("Stripe voucher funding is already bound")
                cur.execute(
                    """
                    UPDATE stripe_voucher_records_v3
        ...
        return self.stripe_voucher_by_purchase(purchase_id)

    def record_stripe_issuance_submission(
        self,
        purchase_id: str,
        *,
        ...
        with self.txn() as cur:
            updated = cur.execute(
                ...
                SET state='ISSUANCE_SUBMITTED', issuance_bundle_id=?,
                    voucher_launcher_id=?, voucher_output_coin_id=?,
                    receipt_coin_id=?, series_input_coin_id=?,
                    series_output_coin_id=?, updated_at=?
                WHERE purchase_id=? AND state='PENDING_ISSUANCE'
            ...
            if updated != 1:
                current = cur.execute(
                    """
                    SELECT issuance_bundle_id FROM stripe_voucher_records_v3
                    WHERE purchase_id=? AND state='ISSUANCE_SUBMITTED'
                    """,
                    (purchase_id.lower(),),
                ).fetchone()
                if (
                    current is None
                    or current["issuance_bundle_id"] != issuance_bundle_id.lower()
                ):
                    raise ValueError(
                        "Stripe voucher submission requires funded pending state"
                    )
        return self.stripe_voucher_by_purchase(purchase_id)

    def confirm_stripe_issuance(
        self,
        purchase_id: str,
        evidence: VoucherIssuanceChainEvidence,
    ) -> dict[str, Any]:
        voucher = self.stripe_voucher_by_purchase(purchase_id)
        series = self._get_series(str(voucher["termsHash"]))
        now = int(time.time())
        with self.txn() as cur:
            chain = cur.execute(
                """
            ...
            if (
                ...
                raise ValueError(
                    "Stripe voucher evidence does not spend current series"
                )
            row = cur.execute(
                """
                SELECT * FROM stripe_voucher_records_v3 WHERE purchase_id=?
            ...
            if row is None or row["state"] != "ISSUANCE_SUBMITTED":
                raise ValueError(
                    "Stripe voucher confirmation requires submitted state"
                )
            expected = {
                ...
                "receipt_coin_id": evidence.payment_commitment_coin_id.lower(),
            ...
            if any(row[field] != value for field, value in expected.items()):
                raise ValueError(
                    "Stripe voucher evidence differs from submitted outputs"
                )
            cur.execute(
                """
                UPDATE stripe_voucher_records_v3
                SET state='ESCROWED', issuance_confirmed_height=?, updated_at=?
                WHERE purchase_id=? AND state='ISSUANCE_SUBMITTED'
                """,
                (
            ...
            if updated != 1:
                raise ValueError(
                    "series state changed during Stripe voucher confirmation"
                )
        return self.stripe_voucher_by_purchase(purchase_id)

    def pending_stripe_terminals(
        self,
    ) -> list[tuple[dict[str, Any], dict[str, Any]]]:
        """Return submitted terminals and LIVE vouchers eligible for delivery."""
        ...
        rows = self._conn.execute(
            ...
            FROM stripe_voucher_records_v3 v
            JOIN presale_series_v2 s ON s.terms_hash=v.terms_hash
            WHERE v.state IN ('REFUNDING','REDEEMING')
               OR (v.state='ESCROWED' AND s.state='LIVE')
    ...
    def record_stripe_terminal_submission(
        self,
        purchase_id: str,
        *,
        ...
        if action not in {
            ...
            raise ValueError("Stripe voucher terminal action is invalid")
        next_state = (
            "REDEEMING"
            if action == VoucherAction.REDEEM
            else "REFUNDING"
        )
        if (action == VoucherAction.REDEEM) != bool(deed_output_coin_id):
            raise ValueError(
                "Stripe voucher deed output must exist only for redemption"
            )
        now = int(time.time())
        exact = {
            "terminal_action": int(action),
            "terminal_evidence_hash": terminal_evidence_hash.lower(),
            "terminal_bundle_id": spend_bundle_id.lower(),
            "terminal_voucher_coin_id": terminal_voucher_coin_id.lower(),
            "terminal_series_input_coin_id": series_input_coin_id.lower(),
            "terminal_series_output_coin_id": series_output_coin_id.lower(),
            "terminal_deed_output_coin_id": (
                deed_output_coin_id.lower()
                if deed_output_coin_id is not None
                else None
            ),
        }
        with self.txn() as cur:
            ...
            if row is None:
                raise KeyError(purchase_id)
            if row["state"] in {"REFUNDING", "REDEEMING"}:
                if row["state"] != next_state or any(
                    row[field] != value for field, value in exact.items()
                ):
                    raise ValueError(
                        "Stripe voucher terminal is bound to different evidence"
                    )
                return self._render_stripe_voucher(
                    row,
                    self._get_series(str(row["terms_hash"])),
            ...
            if row["state"] != "ESCROWED":
                raise ValueError(
                    "only an escrowed Stripe voucher can enter a terminal path"
                )
            cur.execute(
                """
                UPDATE stripe_voucher_records_v3
                SET state=?, terminal_action=?, terminal_evidence_hash=?,
                    terminal_bundle_id=?, terminal_voucher_coin_id=?,
                    terminal_series_input_coin_id=?,
                    terminal_series_output_coin_id=?,
                    terminal_deed_output_coin_id=?, updated_at=?
                WHERE purchase_id=? AND state='ESCROWED'
                """,
                (
                    next_state,
                    exact["terminal_action"],
                    exact["terminal_evidence_hash"],
                    exact["terminal_bundle_id"],
                    exact["terminal_voucher_coin_id"],
                    exact["terminal_series_input_coin_id"],
                    exact["terminal_series_output_coin_id"],
                    exact["terminal_deed_output_coin_id"],
                    now,
                    purchase_id.lower(),
                ),
            )
    ...
    def confirm_stripe_terminal(
        ...
        voucher = self.stripe_voucher_by_purchase(purchase_id)
        series = self._get_series(str(voucher["termsHash"]))
        action = VoucherAction(evidence.action)
        expected_state = (
            "REDEEMING"
            if action == VoucherAction.REDEEM
            else "REFUNDING"
        ...
        if (action == VoucherAction.REDEEM) != bool(
            evidence.deed_output_coin_id
        ):
            raise ValueError(
                "Stripe voucher terminal evidence has an invalid deed output"
            )
        now = int(time.time())
        with self.txn() as cur:
            chain = cur.execute(
            ...
            if row is None:
                raise KeyError(purchase_id)
            if row["state"] in {"REFUNDED", "REDEEMED"}:
                if (
                    row["state"] != final_state
                    or row["terminal_action"] != int(action)
                    or row["terminal_evidence_hash"]
                    != evidence.terminal_evidence_hash.lower()
                    or row["terminal_bundle_id"]
                    != evidence.spend_bundle_id.lower()
                    or row["terminal_voucher_coin_id"]
                    != evidence.terminal_voucher_coin_id.lower()
                    or row["terminal_series_input_coin_id"]
                    != evidence.series_input_coin_id.lower()
                    or row["terminal_series_output_coin_id"]
                    != evidence.series_output_coin_id.lower()
                    or row["terminal_deed_output_coin_id"]
                    != (
                        evidence.deed_output_coin_id.lower()
                        if evidence.deed_output_coin_id is not None
                        else None
                    )
                    or row["terminal_confirmed_height"]
                    != evidence.confirmed_height
                ):
                    raise ValueError(
                        "Stripe voucher terminal confirmation changed"
                    )
                return self._render_stripe_voucher(row, series)
            if (
                chain is None
                ...
                raise ValueError(
                    "Stripe voucher terminal evidence differs from submission"
                )
            cur.execute(
                """
                UPDATE stripe_voucher_records_v3
                SET state=?, terminal_confirmed_height=?, updated_at=?
            ...
            updated = cur.execute(
                ...
                SET current_coin_id=?, current_inner_puzzle_hash=?,
                    lineage_parent_name=?, lineage_inner_puzzle_hash=?,
                    {count_column}={count_column}+1, updated_at=?
                WHERE terms_hash=? AND current_coin_id=?
                """,
                (
                    evidence.series_output_coin_id.lower(),
            ...
            if updated != 1:
                raise ValueError(
                    "series state changed during Stripe voucher terminal confirmation"
                )
        return self.stripe_voucher_by_purchase(purchase_id)
    ...
    def _render_stripe_voucher(
        self, row: sqlite3.Row, series: dict[str, Any]
    ) -> dict[str, Any]:
        commitment = json.loads(row["commitment_json"])
        return {
            "schema": "solslot.stripe-voucher-record.v3",
            "termsHash": row["terms_hash"],
            "seriesSingletonId": series["seriesSingletonId"],
            "seriesState": series["state"],
            "collectionId": series["collectionId"],
            ...
            "paymentPrincipal": commitment["paymentPrincipal"],
            "processingChargeMinor": commitment["processingChargeMinor"],
            "basePriceMinor": commitment["basePriceMinor"],
            "technologyFeeBps": commitment["technologyFeeBps"],
            "technologyFeeMinor": commitment["technologyFeeMinor"],
            "grossPriceMinor": commitment["grossPriceMinor"],
            "originalPayer": row["original_payer"],
            "vaultLauncherId": row["vault_launcher_id"],
            "vaultP2PuzzleHash": row["vault_p2_puzzle_hash"],
            "purchaseId": row["purchase_id"],
            ...
            "stripeReceiptHash": row["stripe_receipt_hash"],
            "state": row["state"],
            "paymentEvidenceId": row["payment_evidence_id"],
            "fundingBundleId": row["funding_bundle_id"],
            "purchaseLauncherCoinId": row["purchase_launcher_coin_id"],
            "purchaseLauncherPuzzleHash": row[
                "purchase_launcher_puzzle_hash"
            ],
            "issuanceBundleId": row["issuance_bundle_id"],
            "voucherLauncherId": row["voucher_launcher_id"],
            "voucherOutputCoinId": row["voucher_output_coin_id"],
            "receiptCoinId": row["receipt_coin_id"],
            "paymentCommitmentCoinId": row["receipt_coin_id"],
            "seriesInputCoinId": row["series_input_coin_id"],
            "seriesOutputCoinId": row["series_output_coin_id"],
            "issuanceConfirmedHeight": row["issuance_confirmed_height"],
            "terminalAction": row["terminal_action"],
            "terminalEvidenceHash": row["terminal_evidence_hash"],
            "terminalBundleId": row["terminal_bundle_id"],
            "terminalVoucherCoinId": row["terminal_voucher_coin_id"],
            "terminalSeriesInputCoinId": row[
                "terminal_series_input_coin_id"
            ],
            "terminalSeriesOutputCoinId": row[
                "terminal_series_output_coin_id"
            ],
            "terminalDeedOutputCoinId": row[
                "terminal_deed_output_coin_id"
            ],
            "terminalConfirmedHeight": row[
                "terminal_confirmed_height"
            ],
            "refundDeadline": series["terms"]["refundDeadline"],
            "deliveryDeadline": series.get("deliveryDeadline"),
            "createdAt": row["created_at"],
...
@router.post(
    "/{terms_hash}/vouchers/{serial}/stripe-refund-request",
    response_model=PrepareStripeVoucherRefundResponse,
)
async def request_stripe_voucher_refund(
    terms_hash: str,
    serial: int,
    request: Request,
    store: Annotated[PresaleStore, Depends(get_presale_store)],
    settings: Annotated[Settings, Depends(get_settings)],
) -> PrepareStripeVoucherRefundResponse:
    """Prepare the one current-vault signature for a Stripe voucher refund."""

    require_presale_writes(settings)
    voucher = response_or_404(
        lambda: store.stripe_voucher(terms_hash, serial)
    )
    session = verify_vault_session(
        settings,
        request,
        voucher["vaultLauncherId"],
    )
    approved = require_current_approved_vault(
        settings,
        session.vault_launcher_id,
    ...
    now = int(time.time())
    if voucher["state"] != "ESCROWED":
        raise HTTPException(
            status_code=409,
            detail="This Stripe voucher is not available for cancellation.",
        )
    if series["state"] == "PRESALE":
        if now >= int(series["terms"]["refundDeadline"]):
            raise HTTPException(
                status_code=409,
                detail="The presale refund window has closed.",
            )
    elif series["state"] != "CANCELED":
        raise HTTPException(
            status_code=409,
            detail="The voucher is waiting for delivery and cannot be canceled.",
        )
    purchases = get_payment_purchase_store(
        settings.payment_purchase_db_path
    )
    try:
        operation = purchases.get_operation(voucher["purchaseId"])
    except PaymentPurchaseNotFound as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if operation.state != PurchaseOperationState.VOUCHER_ESCROWED:
        raise HTTPException(
            status_code=409,
            detail="The Stripe purchase is not in refundable voucher custody.",
        )
    voucher_coin_id = _b32(voucher["voucherOutputCoinId"], nonzero=True)
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return PrepareStripeVoucherRefundResponse(
        termsHash=series["termsHash"],
        serial=serial,
        ...
        seriesCoinId=str(series["chainState"]["currentCoinId"]),
...
@router.post(
    "/{terms_hash}/vouchers/{serial}/stripe-refund-complete",
    response_model=CompleteVoucherRefundResponse,
)
async def complete_stripe_voucher_refund(
    terms_hash: str,
    serial: int,
    body: CompleteStripeVoucherRefundRequest,
    request: Request,
    store: Annotated[PresaleStore, Depends(get_presale_store)],
    settings: Annotated[Settings, Depends(get_settings)],
) -> CompleteVoucherRefundResponse:
    """Burn the exact voucher receipt before any Stripe refund is allowed."""
    ...
    if abs(now - body.current_timestamp) > REFUND_AUTH_MAX_AGE_SECONDS:
        raise HTTPException(
            status_code=409,
            detail="Refund authorization expired.",
        )
    if (
        body.voucher_coin_id.lower()
        != str(voucher["voucherOutputCoinId"]).lower()
        or body.series_coin_id.lower()
        != str(series["chainState"]["currentCoinId"]).lower()
    ):
        raise HTTPException(
            status_code=409,
    ...
    action = (
        VoucherAction.REFUND_PRESALE
        if series["state"] == "PRESALE"
        else VoucherAction.REFUND_CANCELED
        if series["state"] == "CANCELED"
        else None
    )
    if action is None or (
        ...
        raise HTTPException(
            status_code=409,
            detail="This Stripe voucher is not currently refundable.",
        )
    purchases = get_payment_purchase_store(
        settings.payment_purchase_db_path
    )
    try:
        ...
        if operation.state == PurchaseOperationState.VOUCHER_ESCROWED:
            ...
            request_hash = _hex32(
                Program.to(
                    [
                        b"SOLSLOT_STRIPE_PRESALE_REFUND_REQUEST_V1",
                        _b32(voucher["purchaseId"]),
                        _b32(voucher["commitmentHash"]),
                        _b32(voucher["vaultLauncherId"]),
                        body.current_timestamp,
                    ]
                ).get_tree_hash()
            )
            operation = purchases.transition_operation(
                operation.purchase_id,
                expected_revision=operation.revision,
                to_state=PurchaseOperationState.REFUND_PENDING,
                actor="authenticated-vault-owner",
                reason="refundable Stripe voucher cancellation authorized",
                evidence={"refundRequestHash": request_hash},
                changes={
                    "refund_request_hash": request_hash,
                    "refund_requested_at": body.current_timestamp,
                    "last_error": (
                        "consuming voucher receipt before Stripe refund"
                    ),
                },
            )
        elif operation.state != PurchaseOperationState.REFUND_PENDING:
            raise PaymentPurchaseConflict(
                "Stripe voucher purchase is not refundable"
            )
        (
            vault_coin,
            _vault_lineage,
            ...
            voucher_coin_id=_b32(
                voucher["voucherOutputCoinId"],
                nonzero=True,
            ),
        ...
        if worker is None:
            raise ValueError("Stripe voucher settlement worker is unavailable")
        owner_signature = (
            G2Element.from_bytes(
                _hex_bytes(
                    body.owner_authorization,
                    96,
                    "ownerAuthorization",
                )
            )
        ...
        submitted = await worker.submit_stripe_owner_refund(
            series,
            voucher,
            action=action,
            vault_coin=vault_coin,
            ...
            owner_current_timestamp=body.current_timestamp,
            owner_authorization=(
        ...
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    submitted_voucher = submitted["voucher"]
    return CompleteVoucherRefundResponse(
        termsHash=series["termsHash"],
        serial=serial,
        transactionId=submitted_voucher["terminalBundleId"],
        status="MEMPOOL_OBSERVED",
        signerIndices=submitted["signerIndices"],
        voucher=submitted_voucher,
    )
