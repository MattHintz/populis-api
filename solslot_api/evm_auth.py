"""EVM / EIP-712 helpers for the Solslot API.
...
1. **Typed data builders** — emit the exact EIP-712 payload the frontend should
   ...
   no vault coin id exists yet) and one that matches the current in-puzzle
   `SolslotVaultSpend` typehash for actual vault operations.
...
The registration envelope binds owner, nonce, pool launcher, auth type, and
Chia network so neither configuration drift nor cross-network replay can
change what the wallet authorized.
"""
...
# ``eip712_version`` is pinned to "2"; every other domain version is rejected.
REGISTER_TYPES = {
    "EIP712Domain": [
    ...
    REGISTER_PRIMARY_TYPE: [
        ...
        # Bind the user's signature to the exact pool / network /
...
# Mirrors the current in-puzzle vault spend typehash in
# solslot_puzzles/vault_driver.py. Kept as a module constant so tests can
