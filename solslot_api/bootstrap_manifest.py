...
BOOTSTRAP_RECOVERY_ANCHOR_TAG = "SOLSLOT_BOOTSTRAP_V2"
...
def build_bootstrap_artifacts(
    *,
    ...
    _assert_public_artifact(deployment, "deployment_manifest_v2.json")
    _assert_public_artifact(records, "admin_records_v2.json")
    _validate_deployment_header(deployment)
    ...
    if records_launcher != admin_launcher:
        raise BootstrapManifestError(
            "admin_records_v2.json launcher_id does not match admin-authority launcher id"
        )
    ...
    protocol = {
        ...
        "sgt_tail_hash": _hex_from_manifest(deployment, "sgt_tail_hash"),
    ...
    _assert_public_artifact(portal_runtime_config, "portal_runtime_config_v2.json")
    _assert_public_artifact(bootstrap_manifest, "bootstrap_manifest_v2.json")
    _assert_public_artifact(recovery_anchor.payload, "bootstrap_recovery_anchor_v2.json")
...
def build_bootstrap_recovery_anchor(
    *,
    ...
    version: int = BOOTSTRAP_SCHEMA_VERSION,
    ...
    _assert_public_artifact(manifest, "bootstrap_manifest_v2.json")
    _assert_public_artifact(runtime, "portal_runtime_config_v2.json")
    _validate_artifact_header(manifest, "bootstrap_manifest")
    _validate_artifact_header(runtime, "portal_runtime_config")
...
def verify_bootstrap_recovery_artifacts(
    *,
    ...
    payload = _validate_bootstrap_recovery_anchor_payload(bootstrap_recovery_anchor)
    manifest = dict(_require_mapping(bootstrap_manifest, "bootstrap_manifest_v2.json"))
    runtime = dict(_require_mapping(portal_runtime_config, "portal_runtime_config_v2.json"))
    records = dict(_require_mapping(admin_records, "admin_records_v2.json"))
    ...
    if actual_manifest_hash != payload["bootstrap_manifest_hash"]:
        raise BootstrapManifestError(
            "bootstrap_manifest_v2.json content hash does not match recovery anchor"
        )
    actual_runtime_hash = content_hash(runtime)
    if actual_runtime_hash != payload["portal_runtime_config_hash"]:
        raise BootstrapManifestError(
            "portal_runtime_config_v2.json content hash does not match recovery anchor"
        )
    actual_records_hash = content_hash(records)
    if actual_records_hash != payload["admin_records_hash"]:
        raise BootstrapManifestError(
            "admin_records_v2.json content hash does not match recovery anchor"
        )
    ...
    if deployment_manifest is not None:
        deployment = dict(_require_mapping(deployment_manifest, "deployment_manifest_v2.json"))
        _assert_public_artifact(deployment, "deployment_manifest_v2.json")
        _validate_deployment_header(deployment)
        if content_hash(deployment) != deployment_hash:
            raise BootstrapManifestError(
                "deployment_manifest_v2.json content hash does not match bootstrap manifest"
            )
    ...
    if expected_anchor.payload != payload:
        raise BootstrapManifestError(
            "bootstrap_recovery_anchor_v2.json does not match verified bootstrap artifacts"
        )
    ...
    if records_launcher != payload["admin_authority_v2_launcher_id"]:
        raise BootstrapManifestError(
            "admin_records_v2.json launcher_id does not match recovery anchor"
        )
    try:
        ...
        raise BootstrapManifestError(
            f"admin_records_v2.json does not match verified admin authority: {e}"
        ) from e
...
def persist_bootstrap_artifacts(
    *,
    ...
    records = dict(admin_records)
    _assert_public_artifact(records, "admin_records_v2.json")
    _assert_public_artifact(artifacts.portal_runtime_config, "portal_runtime_config_v2.json")
    _assert_public_artifact(artifacts.bootstrap_recovery_anchor, "bootstrap_recovery_anchor_v2.json")
    _assert_public_artifact(artifacts.bootstrap_manifest, "bootstrap_manifest_v2.json")
...
def _validate_bootstrap_recovery_anchor_payload(value: Mapping[str, Any]) -> dict[str, Any]:
    payload = dict(value)
    _assert_public_artifact(payload, "bootstrap_recovery_anchor_v2.json")
...
def _validate_recovery_anchor_version(value: object) -> int:
    if value != BOOTSTRAP_SCHEMA_VERSION:
        raise BootstrapManifestError(
            f"recovery anchor version must be {BOOTSTRAP_SCHEMA_VERSION}"
        )
    return value


def _validate_artifact_header(value: Mapping[str, Any], label: str) -> None:
    if value.get("schemaVersion") != BOOTSTRAP_SCHEMA_VERSION:
        raise BootstrapManifestError(
            f"{label}.schemaVersion must be {BOOTSTRAP_SCHEMA_VERSION}"
        )
    if value.get("protocolVersion") != PROTOCOL_VERSION:
        raise BootstrapManifestError(
            f"{label}.protocolVersion must be {PROTOCOL_VERSION!r}"
        )


def _validate_deployment_header(value: Mapping[str, Any]) -> None:
    if value.get("protocol_version") != PROTOCOL_VERSION:
        raise BootstrapManifestError(
            f"deployment_manifest.protocol_version must be {PROTOCOL_VERSION!r}"
        )
    if value.get("pool_puzzle_version") != 3:
        raise BootstrapManifestError("deployment_manifest.pool_puzzle_version must be 3")
    if value.get("smart_deed_puzzle_version") != 2:
        raise BootstrapManifestError(
            "deployment_manifest.smart_deed_puzzle_version must be 2"
        )
