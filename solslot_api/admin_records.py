"""On-chain-verified Solslot V2 admin records loader.

The Solslot admin desk's authority is held by an on-chain
...
* When ``SOLSLOT_ADMIN_RECORDS_PATH`` is set and the JSON-derived
  ``admins_hash`` matches the operator-supplied
  ``SOLSLOT_PROTOCOL_ADMIN_AUTHORITY_V2_ADMINS_HASH`` env (which
  itself reflects the on-chain singleton state), this records list is
  the only source that can gate ``/admin/*`` routes.
...
The current loader accepts only implemented leaf kinds and rejects every
unsupported authority method before startup completes.
"""
...
# Re-use the canonical hash function from solslot_protocol so we
...
class AdminRecordSpec:
    ...
    Maps 1:1 to ``solslot_protocol.admin_authority_v2_driver.AdminRecord``
...
class AdminRecordsConfig:
    ...
    Loaded from ``SOLSLOT_ADMIN_RECORDS_PATH`` JSON file.  The
    ...
    schema_version: int
    """Schema version.  The clean Solslot ceremony requires version 2;
    ...
    against ``SOLSLOT_PROTOCOL_ADMIN_AUTHORITY_V2_LAUNCHER_ID``.
    """
...
def load_admin_records_from_mapping(
    ...
    if schema_version != 2:
        raise AdminRecordsLoadError(
            f"{label}: unsupported schema version {schema_version!r} "
            f"(supported: 2)"
        )
    if "version" in raw:
        raise AdminRecordsLoadError(
            f"{label}: retired version field is not accepted; use schemaVersion"
        )
...
def _parse_leaf(raw: object, record_pos: int, leaf_pos: int) -> Eip712LeafSpec:
    ...
    # the schema.
    from solslot_puzzles.eip712_helpers import compute_eip712_member_leaf_hash
    try:
...
def verify_against_launcher_id(
    ...
    if config.launcher_id != expected:
        raise AdminRecordsDriftError(
            ...
            f"  env var SOLSLOT_PROTOCOL_ADMIN_AUTHORITY_V2_LAUNCHER_ID: "
...
def verify_admin_records_for_settings(
    ...
    if not expected_hash_hex:
        raise AdminRecordsDriftError(
            "SOLSLOT_PROTOCOL_ADMIN_AUTHORITY_V2_ADMINS_HASH is required "
