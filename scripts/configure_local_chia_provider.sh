...
Options:
  ...
  --primary-url URL     Chia RPC URL (default: https://127.0.0.1:<rpc-port>)
  --existing-tls        Require an enrolled mTLS client in <shared-root>/tls/chia
...
while (($#)); do
  case "$1" in
    ...
    --primary-url) PRIMARY_URL="${2:?missing URL}"; shift 2 ;;
    --existing-tls) EXISTING_TLS=1; shift ;;
...
if [[ -z "$PRIMARY_URL" ]]; then
  PRIMARY_URL="https://127.0.0.1:$RPC_PORT"
fi
if [[ ! "$PRIMARY_URL" =~ ^https://(127\.0\.0\.1|localhost):[0-9]+$ ]]; then
  echo "Primary Chia RPC must remain on loopback: $PRIMARY_URL" >&2
  exit 1
fi
primary_port="${PRIMARY_URL##*:}"
if ((primary_port < 1 || primary_port > 65535)); then
  echo "Invalid primary Chia RPC port: $primary_port" >&2
  exit 1
fi
...
if ((EXISTING_TLS)); then
  for path in "$CLIENT_CA" "$CLIENT_CERT" "$CLIENT_KEY"; do
    if [[ ! -r "$path" ]]; then
      echo "Required enrolled Chia TLS file is unavailable: $path" >&2
      exit 1
    fi
  done
  openssl verify -CAfile "$CLIENT_CA" "$CLIENT_CERT" >/dev/null
  openssl x509 -checkend 2592000 -noout -in "$CLIENT_CERT"
  chown root:"$SERVICE_USER" "$CLIENT_CA" "$CLIENT_CERT" "$CLIENT_KEY"
  chmod 0644 "$CLIENT_CA" "$CLIENT_CERT"
  chmod 0640 "$CLIENT_KEY"
  for path in "$CLIENT_CA" "$CLIENT_CERT" "$CLIENT_KEY"; do
    if ! runuser -u "$SERVICE_USER" -- test -r "$path"; then
      echo "API service user cannot read enrolled Chia TLS file: $path" >&2
      exit 1
    fi
  done
elif [[ -z "$CHIA_ROOT" ]]; then
  mapfile -t roots < <(
...
python3 - "$CLIENT_CA" "$CLIENT_CERT" "$CLIENT_KEY" "$PRIMARY_URL" "$NETWORK" <<'PY'
import json
import ssl
import sys
import urllib.request

ca, cert, key, base_url, expected_network = sys.argv[1:]
...
set_env SOLSLOT_CHIA_PRIMARY_URL "$PRIMARY_URL"
